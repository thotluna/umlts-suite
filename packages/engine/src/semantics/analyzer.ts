...
import type { IRDiagram, IREntity, IRRelationship, IRMember } from '../generator/ir/models'
import type { MemberNode, AttributeNode, MethodNode, ParameterNode } from '../parser/ast/nodes'
...
export class SemanticAnalyzer {
  ...
  public analyze(program: ProgramNode): IRDiagram {
    ...
    const relVisitor = new RelationshipVisitor(
      this,
      this.symbolTable,
      this.relationships,
  ...
  private shouldCreateInferredRel(
    from: string,
    to: string,
    type: IRRelationshipType,
    label?: string,
  ): boolean {
    const exactMatch = this.relationships.some(
      (rel) =>
        rel.from === from && rel.to === to && rel.type === type && rel.label === (label || ''),
    )
    if (exactMatch) return false

    if (type === IRRelationshipType.DEPENDENCY) {
      const strongerExists = this.relationships.some(
        (rel) => rel.from === from && rel.to === to && rel.type !== IRRelationshipType.DEPENDENCY,
      )
  ...
  private cleanupRedundantMembers(): void {
    ...
    entities.forEach((entity) => {
      ...
      entity.members = entity.members.filter((member) => {
        ...
        const isRedundant = outgoingRels.some(
          (rel) =>
...
class DeclarationVisitor implements ASTVisitor {
  ...
  visitEntity(node: EntityNode): void {
    ...
    const entity: IREntity = {
      ...
      isImplicit: false,
      isAbstract: node.type === ASTNodeType.CLASS && node.isAbstract === true,
      isStatic: node.type === ASTNodeType.CLASS && node.isStatic === true,
  ...
  visitRelationship(_node: RelationshipNode): void {
    /* No-op: las relaciones se procesan en la segunda pasada */
  }
  ...
  private mapMembers(members: MemberNode[]): IRMember[] {
    return members
      .filter((m): m is AttributeNode | MethodNode => m.type !== ASTNodeType.COMMENT)
      .map((m) => {
        ...
        return {
          name: m.name,
          type: isAttribute
            ? (m as AttributeNode).typeAnnotation?.raw
            : (m as MethodNode).returnType?.raw,
          visibility: this.mapVisibility(m.visibility),
          isStatic: m.isStatic || false,
          isAbstract: isMethod ? (m as MethodNode).isAbstract : false,
          parameters: isMethod
            ? (m as MethodNode).parameters?.map((p: ParameterNode) => ({
                name: p.name,
