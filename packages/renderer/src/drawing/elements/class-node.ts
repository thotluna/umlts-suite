...
export function renderClassNode(node: UMLNode, theme: Theme, options?: any): string {
  ...
  let activeLines = '';
  if (node.isActive) {
    activeLines += svg.line(x + 5, y, x + 5, y + height, { stroke: theme.nodeActiveLine, 'stroke-width': 1 });
    activeLines += svg.line(x + width - 5, y, x + width - 5, y + height, { stroke: theme.nodeActiveLine, 'stroke-width': 1 });
  }
  ...
  return svg.g({ class: 'node', 'data-id': node.id, 'cursor': 'pointer' },
